...
export async function POST(request: NextRequest) {
  try {
    ...
    // Seconda analisi: AI analysis per suggerimenti
    const aiAnalysis = await performAIAnalysis(enrichedCards, format, optimization_goals)
...
function estimateCardType(cardName: string): string {
  ...
  if (name.includes('bolt') || name.includes('shock')) return 'Instant'
  if (name.includes('counter')) return 'Instant'
  if (name.includes('angel') || name.includes('dragon') || name.includes('elves')) return 'Creature'
  if (name.includes('ring') || name.includes('sol')) return 'Artifact'
  if (name.includes('growth')) return 'Instant'
  if (name.includes('ritual')) return 'Instant'
...
function estimateRarity(cardName: string): string {
  ...
  const rares = ['serra angel', 'shivan dragon', 'force of will']
  const mythics = ['sol ring'] // In realtà è uncommon ma per test
  
  if (mythics.some(r => name.includes(r))) return 'mythic'
  if (rares.some(r => name.includes(r))) return 'rare'
  if (name.includes('bolt') || name.includes('growth')) return 'common'
...
async function performAIAnalysis(
  ...
  try {
    ...
    const prompt = `
Sei un esperto deck builder di Magic The Gathering. Analizza questo deck e fornisci suggerimenti per ottimizzarlo.
...
Rispondi SOLO con JSON valido, nessun testo aggiuntivo.`
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'gpt-4',
        messages: [
          { role: 'system', content: 'You are a Magic: The Gathering deck building expert. Always respond with valid JSON only.' },
