...
export default function FindCombosFromCardsPage() {
  ...
  const findCombos = async () => {
    ...
    if (validCards.length < 1) {
      setError('Inserisci almeno 1 nome di carta')
      return
    }
    ...
    try {
      // Analyze each card individually and collect all combos
      const allCombos: ComboResult[] = []
      const analyzedCards: string[] = []
      let totalCombosFound = 0

      for (const cardName of validCards) {
        try {
          console.log(`Analyzing combos for: ${cardName}`)
          ...
          if (data.ok) {
            analyzedCards.push(cardName)
            
            // Convert API format to our ComboResult format
            const convertedCombos = (data.combos || []).map((combo: any) => ({
              id: combo.id,
              cards: combo.cards.map((c: any) => c.name),
              description: combo.description,
              steps: combo.explanation || [],
              power_level: combo.power_level,
              reliability: combo.reliability,
              category: combo.category,
              synergy_score: Math.min(10, combo.power_level + (combo.keywords_matched?.length || 0)),
              explanation: combo.explanation?.join(' ') || combo.description,
              source_card: cardName
            }))

            // Filter by power level
            const filteredCombos = convertedCombos.filter((combo: ComboResult) => 
              combo.power_level >= filters.min_power_level
            )

            allCombos.push(...filteredCombos)
            totalCombosFound += data.combos.length
            
            console.log(`Found ${filteredCombos.length}/${data.combos.length} combos for ${cardName}`)
          } else {
            console.warn(`No combos found for ${cardName}: ${data.error}`)
          }
        } catch (cardError) {
          console.warn(`Error analyzing ${cardName}:`, cardError)
        }
      }

      // Remove duplicates based on combo description and cards
      const uniqueCombos = allCombos.filter((combo, index, self) => 
        index === self.findIndex(c => 
          c.description === combo.description && 
          JSON.stringify(c.cards.sort()) === JSON.stringify(combo.cards.sort())
        )
      )

      // Sort by power level and synergy score
      const sortedCombos = uniqueCombos.sort((a, b) => {
      ...
      // Generate suggestions based on found combos
      const suggestions = generateSuggestions(sortedCombos, validCards)

      setResults({
        combos: sortedCombos,
        suggestions,
        analysis: {
          cards_analyzed: analyzedCards.length,
          total_combos_found: totalCombosFound,
          unique_combos: sortedCombos.length,
          cards_with_combos: analyzedCards
        }
      })

      if (sortedCombos.length === 0) {
        setError(`Nessuna combo trovata per le carte analizzate: ${analyzedCards.join(', ')}`)
      }

    } catch (err) {
      console.error('Find combos error:', err)
      setError('Errore durante la ricerca combo')
    } finally {
      setLoading(false)
    }
  }

  const generateSuggestions = (combos: ComboResult[], inputCards: string[]): SuggestionResult => {
    if (combos.length === 0) {
      return {
        suggested_cards: ['Lightning Bolt', 'Counterspell', 'Sol Ring'],
        reasoning: 'Prova con carte più generiche che si sinergizzano facilmente',
        potential_combos: 0,
        power_increase: 0
      }
    }

    // Find most common cards in combos that aren't in input
    const cardFrequency: { [card: string]: number } = {}
    
    combos.forEach(combo => {
      combo.cards.forEach(card => {
        if (!inputCards.some(input => 
    ...
    const suggestedCards = Object.entries(cardFrequency)
      .sort(([,a], [,b]) => b - a)
      .slice(0, 5)
      .map(([card]) => card)
    ...
    return {
      ...
      reasoning: `Queste carte appaiono frequentemente nelle combo trovate e potrebbero aumentare le sinergie`,
      potential_combos: Math.floor(combos.length * 1.3),
      power_increase: Math.min(3, Math.floor(combos.length / 3))
    }
  }
